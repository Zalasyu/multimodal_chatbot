...
def main():
    ...
    # video_segment_models: list[VideoSegmentModel] = [db.video_segment_data_to_model(segment=segment) for segment in video_data.segments]
    ...
    # Open the tables
    video_segments_table = db.db.open_table(name="VideoSegments")
    # video_segments_table.create_fts_index("enriched_transcript", use_tantivy=False)

    # Peek into the table
    logger.debug(f"Display Table head: {video_segments_table.to_pandas()[['transcript', 'extracted_frame_path']].head()}")

    # Query Test
    query = "What did ChatGPT do with Claude?"
